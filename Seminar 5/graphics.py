...
# create de correlogram graphic
def correlogram(R2, title='Correlogram',
                dec=2, minVal=-1,
                maxVal=1):  # R2 can be either ndarray od DataFrame
    plt.figure(num=title, figsize=(8, 8))
    plt.title(title, fontsize=12, verticalalignment='bottom')
    sb.heatmap(data=np.round(R2, decimals=dec), vmin=minVal,
               vmax=maxVal, cmap='bwr', annot=True)


def correlationCircle(R2, k1=0, k2=1, dec=2,
    ...
    if isinstance(R2, np.ndarray):
        plt.xlabel(xlabel='Variable ' + str(k1+1),
            fontsize=10, color='Blue', verticalalignment='top')
        plt.ylabel(ylabel='Variable ' + str(k2 + 1),
           fontsize=10, color='Blue', verticalalignment='top')
        ...
        plt.xlabel(xlabel='Variable ' + R2.columns.values[k1],
                   fontsize=10, color='Blue', verticalalignment='top')
        plt.ylabel(ylabel='Variable ' + R2.columns.values[k2],
                   fontsize=10, color='Blue', verticalalignment='top')
...
def explainedVariance(alpha,
    ...
    plt.xlabel(xlabel='Principal Components',
               fontsize=10, color='Blue', verticalalignment='top')
    plt.ylabel(ylabel='Eigenvalues - explained variance',
